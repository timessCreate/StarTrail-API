...
-- 接口信息表
create table if not exists api_interface.interface_info
(
...
-- 用户调用接口关系表
create table if not exists api_interface.user_interface_info
(
    id                 bigint auto_increment comment '主键'  primary key,
    userId             bigint             not null comment '调用者id',
    interfaceInfoId 	 bigint             not null comment'被调用接口id',
    totalNum 				   int     default 0  not null comment '总调用次数',
    leftNum 					 int     default 0  not null comment '该接口剩余调用次数',
    status             int     default 0  not null comment '0-正常， 1-禁用',
    createTime     datetime default CURRENT_TIMESTAMP not null comment '首次调用时间',
    updateTime     datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间',
    isDelete       tinyint  default 0                 not null comment '是否删除(0-未删除, 1-已删除)'
) comment '用户调用接口关系表';
